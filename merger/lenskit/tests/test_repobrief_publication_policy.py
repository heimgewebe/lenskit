...
import merger.lenskit.core.repobrief_publication_policy as publication_policy

from merger.lenskit.core.repobrief_publication_policy import (
    PublicationIdentity,
...
def test_cli_round_trip_enforces_identity_noop(tmp_path: Path) -> None:
    script = (
        Path(__file__).resolve().parents[3] / "scripts" / "ops" / "rb-publication-policy"
    )
