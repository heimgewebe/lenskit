...
def test_eval_graph_delta_reporting(mini_index_for_eval, tmp_path):
    # Setup test graph
    valid_graph = {
    ...
    # Check that delta metrics are present since graph_index_path was provided
    assert "graph_recall@5" in metrics
    assert "delta_graph_recall" in metrics
    assert "delta_graph_mrr" in metrics
    assert "baseline_recall@5" in metrics

    # In this dataset, baseline and graph should both find login.py (RR=1.0, Recall=100)
    assert metrics["baseline_recall@5"] == 100.0
    assert metrics["graph_recall@5"] == 100.0
    assert metrics["delta_graph_recall"] == 0.0
