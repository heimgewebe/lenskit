...
SCHEMA = ROOT / "merger/lenskit/contracts/repobrief-readonly-adapter-config.v1.schema.json"
...
def _adapter(tmp_path: Path) -> tuple[RepoBriefReadonlyAdapter, dict, Path]:
    ...
    return RepoBriefReadonlyAdapter.from_config(config), bundle, config
...
def test_adapter_exposes_only_registered_manifest_identity(tmp_path: Path) -> None:
    ...
    with pytest.raises(RepoBriefReadonlyAdapterError, match="not registered"):
...
def test_adapter_rejects_manifest_outside_allowed_root(tmp_path: Path) -> None:
    ...
    with pytest.raises(RepoBriefReadonlyAdapterError, match="escapes allowed roots"):
        RepoBriefReadonlyAdapter.from_config(config)
