...
def test_atlas_search_content(tmp_path):
    registry_path = tmp_path / "registry.sqlite"
    registry = AtlasRegistry(registry_path)

    # Set up machine and root pointing to tmp_path
    registry.register_machine("m1", "host1")
    registry.register_root("r1", "m1", "abs_path", str(tmp_path))

    # Create dummy text file
    dummy_file = tmp_path / "dummy.txt"
    dummy_file.write_text("This is some test content with a SPECIAL_KEYWORD inside.")

    # Create inventory containing dummy.txt
    inv_path = tmp_path / "inv.jsonl"
    with open(inv_path, "w") as f:
        f.write(json.dumps({"rel_path": "dummy.txt", "name": "dummy.txt", "ext": ".txt", "size_bytes": dummy_file.stat().st_size, "mtime": "2023-01-01T00:00:00Z", "is_text": True}) + "\n")
    ...
    # Test search finding content
    res = searcher.search(content_query="SPECIAL_KEYWORD")
    assert len(res) == 1
    assert res[0]["name"] == "dummy.txt"
    assert "snippet" in res[0]
    assert "SPECIAL_KEYWORD" in res[0]["snippet"]

    # Test case insensitivity
    res_lower = searcher.search(content_query="special_keyword")
    assert len(res_lower) == 1

    # Test non-matching query
    res_none = searcher.search(content_query="NON_EXISTENT")
