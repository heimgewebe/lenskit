...
def _valid_card(path: str = "merger/lenskit/contracts/lens-card.v1.schema.json") -> dict:
...
def test_schema_validates_minimal_card() -> None:
    ...
    assert card["path"] == "merger/lenskit/contracts/lens-card.v1.schema.json"
...
def test_schema_validates_card_with_empty_facet_list() -> None:
    card = _valid_card("merger/lenskit/core/lenses.py")
...
@pytest.mark.parametrize(
    ...
    [
        ("merger/lenskit/core/lenses.py", "core", []),
        (
            "merger/lenskit/contracts/lens-card.v1.schema.json",
            "data_models",
            ["contract"],
        ),
        ("merger/lenskit/tests/test_lens_cards.py", "guards", ["test"]),
        ("merger/lenskit/retrieval/review_eval.py", "core", ["retrieval"]),
...
def test_same_input_yields_same_card() -> None:
    path = "merger/lenskit/retrieval/test_eval_capability.py"
...
def test_batch_deduplicates_paths() -> None:
    paths = [
        "merger/lenskit/core/lenses.py",
        PurePosixPath("merger/lenskit/core/lenses.py"),
        "merger/lenskit/core/lenses.py",
    ]
    cards = produce_lens_cards(paths)
    assert [card["path"] for card in cards] == ["merger/lenskit/core/lenses.py"]


def test_batch_accepts_generator() -> None:
    paths = (path for path in ["merger/lenskit/core/lenses.py"])
...
@pytest.mark.parametrize(
    ...
    [
        "merger/lenskit/core/lenses.py",
        b"merger/lenskit/core/lenses.py",
        bytearray(b"merger/lenskit/core/lenses.py"),
        PurePosixPath("merger/lenskit/core/lenses.py"),
        Path("merger/lenskit/core/lenses.py"),
        PureWindowsPath("merger/lenskit/core/lenses.py"),
    ],
)
...
def test_batch_does_not_mutate_input_list() -> None:
    paths = ["merger/lenskit/core/lenses.py"]
    original = list(paths)
...
def test_bad_element_inside_batch_is_not_swallowed() -> None:
    with pytest.raises(ValueError):
        produce_lens_cards(["merger/lenskit/core/lenses.py", "a//b"])
...
def _imports_private_or_rule_engine(py_path: Path) -> list[str]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            ...
            if module.startswith("merger.lenskit.core."):
