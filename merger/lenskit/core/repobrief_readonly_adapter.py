"""Protocol-neutral, fail-closed access to existing RepoBrief bundles.

The adapter is deliberately narrower than the RepoBrief CLI and independent of
...
class RepoBriefReadonlyAdapterError(ValueError):
...
def _json_object(path: Path, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise RepoBriefReadonlyAdapterError(f"{label} does not exist: {path}") from exc
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise RepoBriefReadonlyAdapterError(f"{label} is not valid UTF-8 JSON: {path}") from exc
    if not isinstance(value, dict):
        raise RepoBriefReadonlyAdapterError(f"{label} must be a JSON object: {path}")
    return value
...
def _resolve_config_path(base: Path, raw: Any, *, label: str) -> Path:
    if not isinstance(raw, str) or not raw.strip():
        raise RepoBriefReadonlyAdapterError(f"{label} must be a non-empty path string")
...
def _validate_manifest(path: Path) -> None:
    if not path.is_file() or not path.name.endswith(".bundle.manifest.json"):
        raise RepoBriefReadonlyAdapterError(
            f"snapshot manifest is not a RepoBrief bundle manifest file: {path}"
        )
    document = _json_object(path, label="snapshot manifest")
    if (
        document.get("kind") != "repolens.bundle.manifest"
        or not isinstance(document.get("run_id"), str)
        or not isinstance(document.get("artifacts"), list)
    ):
        raise RepoBriefReadonlyAdapterError(
            f"snapshot manifest has an invalid RepoBrief identity: {path}"
        )
...
class RepoBriefReadonlyAdapter:
    ...
    @classmethod
    def from_config(cls, config_path: str | Path) -> "RepoBriefReadonlyAdapter":
        ...
        if not isinstance(raw_roots, list) or not raw_roots:
            raise RepoBriefReadonlyAdapterError("allowed_roots must be a non-empty array")
        roots = tuple(
        ...
        if not isinstance(raw_snapshots, list) or not raw_snapshots:
            raise RepoBriefReadonlyAdapterError("snapshots must be a non-empty array")
        registrations: list[SnapshotRegistration] = []
        seen_ids: set[str] = set()
        for raw in raw_snapshots:
            if not isinstance(raw, dict):
                raise RepoBriefReadonlyAdapterError("snapshot registration must be an object")
            snapshot_id = raw.get("id")
            if not isinstance(snapshot_id, str) or not snapshot_id.strip():
                raise RepoBriefReadonlyAdapterError("snapshot id must be a non-empty string")
            if snapshot_id in seen_ids:
                raise RepoBriefReadonlyAdapterError(f"duplicate snapshot id: {snapshot_id}")
    ...
    def _registration(self, snapshot_id: Any) -> SnapshotRegistration:
        if not isinstance(snapshot_id, str) or not snapshot_id:
            raise RepoBriefReadonlyAdapterError("snapshot_id must be a non-empty string")
        try:
    ...
    def snapshot_list(self) -> dict[str, Any]:
        ...
        for registration in self._snapshots.values():
            status = repobrief_access.snapshot_status(registration.manifest)
            snapshots.append(
                {
    ...
    def artifact_get(
        ...
        *,
        ...
        if not isinstance(role, str) or not role:
            raise RepoBriefReadonlyAdapterError("role must be a non-empty string")
        result = _base_response(
            "artifact_get",
            status="missing",
            snapshot_id=registration.snapshot_id,
    ...
    def canonical_range_get(self, snapshot_id: Any, range_ref: Any) -> dict[str, Any]:
        ...
        result["range_result"] = repobrief_access.range_get(
    ...
    def required_reading_resolve(
        ...
        result["required_reading"] = repobrief_access.resolve_required_reading_for_bundle(
            registration.manifest,
            task_profile,
        )
    ...
    def dispatch(self, request: Any) -> dict[str, Any]:
        ...
        except (RepoBriefReadonlyAdapterError, ValueError) as exc:
