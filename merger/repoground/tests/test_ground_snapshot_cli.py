...
def _stub_successful_finalization(bundle_manifest: Path, profile: str):
    export_path = cmd_ground.emit_export_safety_report(bundle_manifest, profile)
    evaluation = cmd_ground.mark_bundle_manifest_profile(bundle_manifest, profile)
    return {
        "status": "pass",
        "errors": [],
...
def test_manifest_profile_marker_uses_capabilities_extension(tmp_path):
    ...
    cmd_ground.mark_bundle_manifest_profile(manifest, "agent-portable")
...
def test_external_manifest_refresh_returns_exit_2_for_bundle_generation_resolver_error(
    ...
    def fail_resolver(_path):
        from merger.repoground.core.bundle_generation import BundleGenerationError

        raise BundleGenerationError("current JSON pointer contract mismatch")

    monkeypatch.setattr(cmd_ground, "run_snapshot_create", fake_snapshot_create)
    monkeypatch.setattr(
        "merger.repoground.core.bundle_generation.resolve_bundle_manifest_path",
        fail_resolver,
    )

    rc = cmd_ground.run_external_manifest_refresh(
        Namespace(
            repo=str(repo),
            out=str(out),
...
def test_snapshot_create_json_pointer_outputs_resolved_immutable_manifest(
    ...
    monkeypatch.setattr(
        "merger.repoground.core.bundle_generation._write_current_symlink",
        unsupported_symlink,
    )
...
def test_add_manifest_artifact_preserves_existing_contract_metadata(tmp_path):
    ...
    cmd_ground._add_manifest_artifact(
        manifest,
        artifact,
...
def test_snapshot_finalization_rejects_surface_link_escape(tmp_path):
    ...
    result = cmd_ground.finalize_snapshot_bundle(manifest, "local-private")
