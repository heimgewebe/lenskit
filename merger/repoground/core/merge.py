...
"""
merge_core – Core functions for RepoGround build (v2.4 contract).
...
class ReportValidator:
    ...
    REQUIRED_ORDER = [
        "header",               # # RepoGround Report ... (legacy repoLens accepted)
...
def generate_architecture_summary(files: List[FileInfo]) -> str:
    ...
    lines.append("# RepoGround Architecture Snapshot")
    lines.append("")
...
def write_reports_v2(
    ...
    if md_paths and not verified_md:
        ...
        raise RuntimeError(
            "RepoGround: Report was announced as written, but no non-empty .md output exists on disk. "
    ...
    if extras and extras.json_sidecar:
        ...
        if json_paths and not verified_json:
            raise RuntimeError(
                "RepoGround: JSON primary artifact was announced as written, but no valid non-empty .json exists on disk."
            )
    ...
    from .bundle_identity import CANONICAL_BUNDLE_KIND, CANONICAL_BUNDLE_VERSION
    from .runtime_provenance import build_runtime_provenance
    from .snapshot_provenance import build_snapshot_provenance

    generator_block = {
        "name": generator_info.get("name", "repoground"),
