...
def main() -> None:
    ...
    if TASK_ID not in board:
        row = (
            "| TASK-GRAPH-RESOLUTION-LAYERS-001 | Unique Local Import Resolution and Path Layers | done | "
            "`merger/lenskit/architecture/import_graph.py`, "
            "`merger/lenskit/tests/{test_architecture_import_graph,test_graph_quality_goldset}.py`, "
            "`docs/diagnostics/graph-quality-baseline.{md,v1.json}`, "
            "`docs/proofs/graph-local-resolution-layers-proof.md` | "
            "G4a resolves only uniquely mapped repository-relative Python modules, preserves ambiguous/external modules, "
            "and assigns conservative path-segment layers. Goldset: local 4/4, external 2/2, layers 6/6. "
            "No runtime-causality, graph-completeness, retrieval-benefit, or default-ranking claim. |\n"
        )
    ...
    data["tasks"].append(
        {
            ...
            "status": "done",
            "description": (
                "Implements measured G4a producer improvements: uniquely mapped repository-relative Python modules "
                "become local file edges, ambiguous and unavailable modules remain external, and deterministic "
                "path-segment layers cover test, cli, core and infrastructure files."
            ),
            "evidence": [
                "merger/lenskit/architecture/import_graph.py",
