...
It exists to establish vocabulary boundaries before PR 3B changes schemas, tests, or producer expectations.

It separates:

- what is *schema-valid*;
- what is *producer-emitted by current code*;
- what is *reserved* vocabulary;
- what is *obsolete/unendorsed* planning residue.

## Decision summary

- `minimal_fallback` is reserved vocabulary until a producer emits it.
- `range_ref_minimal` is not an explicit enum value and is not endorsed; BSV may accept it structurally only because its `engine` field is an unconstrained string.
- PR 3B should add positive and negative Golden tests for current OH/PEH diagnostics, not introduce new vocabulary or migrate check shapes.
...
No local sidecars were available in the working tree, so no emitted-sidecar evidence is claimed here; the producer-emitted column is grounded in producer code, with test assertions where present.
...
- Older reports may omit `validation` where the schema allows it.
- Where a `validation` object is present, the checked schemas require `mode`, `engine`, and `reason`.
...
| `unsupported_runtime` | OH, PEH | `post_emit_health.py` | Runtime cannot run the check | Emitted by PEH via `_schema_skip_reason`; OH does not emit it in current code. |
...
- `skipped_unavailable` (a **mode**) is **not** equivalent to `dependency_unavailable` (a **reason**). A `skipped_unavailable` mode can carry `reason` in {`dependency_unavailable`, `schema_missing`, `check_not_applicable`, `unsupported_runtime`} depending on why the check did not run.
...
| `range_ref_minimal` | no explicit enum; BSV accepts any string structurally | none | obsolete/unendorsed unless proven | Not present as an explicit schema enum value, producer value, `Literal` type, or test assertion. Do not use as an OH/PEH example, and do not treat BSV's free string acceptance as semantic endorsement. |
...
Obsolete/unendorsed planning residue:
...
A safe next implementation slice should keep the vocabulary stable and avoid a version bump.

### PR 3B should

- Add or adjust **Golden tests** that pin the currently schema-valid, currently emitted `validation` objects for OH and PEH.
...
- Introduce new `engine` values before schema and producer agreement.
- Claim `doc_freshness_minimal` is emitted before a real producer emits it.
- Claim `minimal_fallback` is emitted without a real producer.
- Introduce `range_ref_minimal`; it must remain unendorsed until explicit schema and producer agreement exists.
- Bundle `checks` shape unification; keep that as the separate **TASK-VALIDATION-DIAG-003**.
