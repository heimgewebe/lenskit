...
|---|---|---|---|---|---|---|
| 1 | Reading Policy: `canonical_md` = Wahrheit, JSON = Navigation | DONE | `core/agent_reading_pack.py:13-16,411-416`; `lenskit-artifact-output-control-plane.md` §2.1; `architecture/artifact-inventory.md` | niedrig | reuse | — |
| 2 | Agent Reading Pack, `authority=navigation_index`/`derived` | DONE | `core/agent_reading_pack.py:381-394`; `core/constants.py:22`; `output-optimierung-v1.md` AP D | niedrig | reuse | — |
| 3 | `TOP_FILES`-Heading vorhanden | DONE | `core/agent_reading_pack.py:489` | mittel (Begriff "TOP" impliziert Wichtigkeit) | harden | A1 |
| 4 | `TOP_FILES` als "wichtig" beschrieben | DRIFT | **README.md:35** "die wichtigsten Quelldateien"; Pack selbst sagt "by chunk coverage" (`agent_reading_pack.py:489-494`) | mittel | resolve (README jetzt; Heading-Rename A1) | A1 |
| 5 | Cache/Tooling-Dirs in canonical/chunk/sqlite | DONE (exkludiert) | `core/merge.py:297-314` (`SKIP_DIRS`), Walk-Filter `merge.py:2277` (#681–#683) | **Plan-Beleg STALE** | reuse + harden | A2 |
| 6 | Reading Lenses markieren Cache als `core` | PARTIAL (latent) | `core/lenses.py:87` (Ultimate-Fallback `core`, keine Exclusion) — Caches erreichen Lens aber nicht (Upstream-Skip) | niedrig-latent | harden (Guard-Test) | A2 |
| 7 | `output_health=pass` trotz `redact_secrets=false` / `agent_pack=skipped` | DONE (bestätigt) | `core/output_health.py:468-469` (Redaction nur protokolliert), `:461-466` (skip = keine Warnung), `:486-491` (Verdict nur aus errors/warnings) | **hoch** (Schein-Agentensicherheit) | harden via separates Gate, **nicht** Health umbiegen | A4/A5 |
| 8 | Post-hoc Agent Bundle Validator | PARTIAL (geplant) | `output_health.py:424-466` (Pfad in-pipeline unset); `control-plane` §2.4 `post_emit_health`; `output-optimierung-v1.md` AP H; `artifact-evidence-levels.md` `post_emit_validation_available` | hoch | reuse Design (`post_emit_health`), **kein** paralleles "agent-ready" | A4 |
| 9 | Safe Output Profiles / agent-safe | DRIFT (doppelt) | `output-optimierung-v1.md` AP E (`max-private/agent-safe/public-review/ci-diagnostic`) **vs** `control-plane.md` §7 (`lean-readable/agent-portable/local-search/debug-full/forensic-strict`) | mittel (Namens-Drift) | resolve (control-plane kanonisch; Plan-Namen als Intent/Alias) | A5 |
| 10 | Range-Ref v2 (Source/Artifact-Achsen trennen) | PARTIAL (Design-only) | `blueprints/range-ref-v2-semantic-boundary-split-preimage.md`; `architecture/range-semantics.md`; nur `contracts/range-ref.v1.schema.json`; `output-optimierung-v1.md` AP B `[ ]` | mittel | reuse Blueprint, Feldnamen angleichen, Schema bauen | A3 |
| 11 | Authority/Canonicality-Felder | DONE | `architecture/artifact-inventory.md` (Authority/Canonicality-Spalten); `bundle-manifest.v1.schema.json`; `AUTHORITY_REGISTRY` in `core/merge.py` | niedrig | reuse/normieren (nicht neu erfinden) | C1 |
...
| 13 | Claim Boundaries / `does_not_prove` | DONE | `contracts/query-result.v1.schema.json:251-293`; `contracts/agent-query-session.v2.schema.json:57-110` (`session_authority`, `claim_boundaries.proves/does_not_prove`) | niedrig | reuse; Muster auf Pack/Context-Bundle ausweiten | A1/B3 |
| 14 | Context Bundle / Agent Session Risk-Felder | PARTIAL | `agent-query-session.v2` DONE (`session_authority`, `context_source`, `claim_boundaries`); `query-context-bundle.v1.schema.json` hat `resolver_status`, **kein** `context_risk`/`claim_boundaries` | mittel | harden (`context_risk` in context-bundle) | B3 |
| 15 | Redaction/Profile-Mechanik | PARTIAL | `core/redactor.py` (heuristischer Redactor); `capabilities.redaction` im Manifest; **keine** Profil-Engine, die Redaction an agent-safe koppelt | mittel | reuse Redactor; Profil-Gate bauen | A5 |
| 16 | Claim-Evidence-Map mit `supported/unsupported` | DRIFT (**Widerspruch**) | `output-optimierung-v1.md` AP F:244 ("supported/unsupported"); `control-plane.md` §4 (`evidence_index`, "beweisnah"); `artifact-evidence-levels.md` `forensic_strict` | **hoch** (Auto-Claim-Bewertung = Scheinsicherheit) | **resolve** (AP F umschreiben: nur Referenz, kein Verdikt) | F1–F3 |
...
| 20 | Context Quality Signals (Artefakt) | MISSING (überlappt) | überlappt `artifact-evidence-levels.md`; kein `context_quality.json` | mittel (globale "Verstehens"-Ampel wäre gefährlich) | defer + scope (Projektion, keine neue Wahrheit) | B1 |
| 21 | Retrieval Miss Taxonomy | MISSING (neu) | `retrieval/eval_core.py` + `contracts/retrieval-eval.v1.schema.json`; keine Miss-Taxonomie | mittel | build über bestehendem Eval | B2 |
| 22 | Structural Architecture Signals | PARTIAL (überlappt) | `output-optimierung-v1.md` AP G; `architecture_summary` | niedrig | in AP G falten, kein Parallelartefakt | E3 |
| 23 | `excluded_noise` als Diagnose | MISSING | Exclusion ist heute **stumm** (`SKIP_DIRS`) | niedrig | optionales Diagnose-Artefakt | A2 |
| 24 | `is_noise_file` vs `SKIP_DIRS` Listendrift | DRIFT | `merge.py:297-314` (hat `.ruff_cache/.pytest_cache/.mypy_cache`) vs `merge.py:1772-1780` (`noisy_dirs` ohne diese) | niedrig-latent | reconcile | A2 |
...
`core/agent_reading_pack.py` markiert den Pack deterministisch als
...
(#681–#683). Aktuell exkludiert `SKIP_DIRS` (`merge.py:297-314`):
...
Walk-Filter wendet sie an (`merge.py:2277`).
→ Hard-Exclusion = `reuse`. Offene Härtung: (a) Guard-Regressionstest, (b)
`is_noise_file`-Liste mit `SKIP_DIRS` versöhnen (`merge.py:1772-1780` fehlen
`.ruff_cache/.pytest_cache/.mypy_cache`), (c) optionales `excluded_noise`-Diagnostikum.
Latentes Restrisiko: `lenses.py:87` fällt im Zweifel auf `core` zurück — nur relevant,
...
(`control-plane.md` §2.4) und die Parity-Gate-Semantik (`master-roadmap.md`
"Paritaetsgates").

### 2.5 Bereits vorhanden: Claim Boundaries & "does_not_prove"
Der Plan behandelt `does_not_prove` als neu. Es existiert:
- `query-result.v1.schema.json:251-293` — `claim_boundaries` mit `does_not_prove`,
  `evidence_basis`, `requires_live_check`.
- `agent-query-session.v2.schema.json:57-110` — `session_authority =
...
`output-optimierung-v1.md` Arbeitspaket F (`:238-247`) verlangt, pro Claim
`supported / unsupported` auszuweisen; `control-plane.md` §4 listet `claim_evidence_map`
als `evidence_index`/"beweisnah"; `artifact-evidence-levels.md` macht es zur Bedingung
für `forensic_strict`. Der vorgelegte Plan verbietet genau das (Invariante 3; F3
verbietet `supported/unsupported/true/false/proven`).
...
`control-plane.md` definiert Profile (`agent-portable`, `local-search`, `debug-full`,
`forensic-strict`, …), Evidence-Level (`readable`→`forensic_strict`), Capability-Matrix
und das Pre/Post-Health-Modell. `artifact-evidence-levels.md` liefert die
...
Es gibt `blueprints/range-ref-v2-semantic-boundary-split-preimage.md` und
`architecture/range-semantics.md` (canonical/source/chunk/semantic getrennt), aber nur
`contracts/range-ref.v1.schema.json`. Die Feldnamen des Plans (z. B.
...
2. Live-Branch-Diffs nach allen Merges vs. lokale Snapshots fehlen (wie in
   `output-optimierung-v1.md` "Epistemische Leere" notiert).
