...
Ziel:
- [x] Jede Agent-Nutzung ist nachvollziehbar (Schema und lokaler Builder existent, CLI-Generierung im realen Pfad über `merger/lenskit/cli/cmd_query.py` implementiert und durch `merger/lenskit/tests/test_cli_agent_session.py` belegt; API-Generierung über `merger/lenskit/service/app.py` implementiert und durch `test_api_query_agent_session_trace` belegt).
...
- [x] 4. test_agent_session_trace_contains_refs (lokaler Builder-Test vorhanden (`merger/lenskit/tests/test_agent_session_builder.py`), CLI-Trace-Test vorhanden (`merger/lenskit/tests/test_cli_agent_session.py`), E2E-API-Test für Session-Erzeugung in `merger/lenskit/tests/test_api_query.py` implementiert)
...
- [x] 5. `agent_query_session.json` (Schema existiert, Builder via `merger/lenskit/retrieval/session.py` existiert, automatische Erzeugung im CLI und API implementiert und testbelegt)
