...
Ziel:
- [~] Jede Agent-Nutzung ist nachvollziehbar (teilweise: Schema und lokaler Builder existent, CLI-Generierung in cmd_query.py vorhanden, API-Generierung fehlt).
...
- [~] 4. test_agent_session_trace_contains_refs (teilweise: lokaler Builder-Test und CLI-Test in test_cli_agent_session.py vorhanden, aber E2E-API-Test offen)
...
- [~] 5. agent_query_session.json (teilweise: Schema und lokaler minimaler Builder via session.py vorhanden, automatische Erzeugung in CLI implementiert, in API noch offen)
