...
Ziel:
- [x] Jede Agent-Nutzung ist nachvollziehbar (teilweise: Schema und lokaler Builder existent, CLI-Generierung im realen Pfad über `merger/lenskit/cli/cmd_query.py` implementiert und durch `merger/lenskit/tests/test_cli_agent_session.py` belegt; API-Generierung implementiert).
...
- [x] 4. test_agent_session_trace_contains_refs (teilweise: lokaler Builder-Test vorhanden (`merger/lenskit/tests/test_agent_session_builder.py`), CLI-Trace-Test vorhanden (`merger/lenskit/tests/test_cli_agent_session.py`), E2E-API-Test für Session-Erzeugung implementiert)
...
- [x] 5. `agent_query_session.json` (teilweise: Schema existiert, Builder via `merger/lenskit/retrieval/session.py` existiert, automatische Erzeugung im CLI implementiert und testbelegt; automatische Erzeugung im API-Pfad implementiert)
