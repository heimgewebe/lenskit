...
| `cross-repo-links.v1.schema.json` | Contract für Cross-Repo-Verknüpfungen (Identitäten zwischen Repositories). | `source_repo`, `target_repo`, `link_type`, `evidence_refs`, `confidence` | `confidence` (enum: `explicit`, `inferred`) | - | Hoch. Contract vorhanden; minimaler heuristischer Runtime-Producer implementiert (`_build_cross_repo_links` in `federation_query.py`): emittiert `co_occurrence`-Links mit `confidence: "inferred"` für jedes eindeutige Repo-Paar mit Treffern; CLI-Persistenz als `cross_repo_links.json` bei `--trace`. Kein globaler Identity-Beweis. Ranking unverändert. Offen: semantisch belastbare Identity-Engine. |
...
-   Formale Schemas für `cross_repo_links.json` und `federation_conflicts.json` sind vorhanden (`cross-repo-links.v1.schema.json`, `federation-conflicts.v1.schema.json`). `federation_conflicts` ist heuristisch/minimal im föderierten Query-/Trace-Pfad emittiert und per CLI-Trace persistierbar. `cross_repo_links` hat nun einen minimalen heuristischen Runtime-Producer (`_build_cross_repo_links`): emittiert `co_occurrence`-Links mit `confidence: "inferred"` pro Repo-Paar; CLI-Persistenz als `cross_repo_links.json` bei `--trace`. Offen bleiben: semantisch belastbare Identity-Engine, durchgängige Nutzung in API-Endpoints, vollständige Integrationspfade.
