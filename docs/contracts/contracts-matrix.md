...
| `cross-repo-links.v1.schema.json` | Contract für Cross-Repo-Verknüpfungen (Identitäten zwischen Repositories). | `source_repo`, `target_repo`, `link_type`, `evidence_refs`, `confidence` | `link_type` (laut Schema) | - | Hoch. Contract vorhanden; Runtime-Producer noch offen. |
| `federation-conflicts.v1.schema.json` | Contract für föderierte Konfliktspuren (Array von Konfliktobjekten). | `conflict_id`, `type`, `involved_results`, `description`, `resolution` | `type` (enum: `identity`, `path`, `symbol`, `evidence`, `provenance`); `resolution` (const: `unresolved`) | - | Mittel bis hoch. Heuristisch/minimal emittiert (Runtime-Emission in `federation_query.py`, CLI-Persistenz in `cmd_federation.py`); semantische Konfliktlogik noch offen. |
