...
| `cross-repo-links.v1.schema.json` | Contract für Cross-Repo-Verknüpfungen (Identitäten zwischen Repositories). | `source_repo`, `target_repo`, `link_type`, `evidence_refs`, `confidence` | `confidence` (enum: `explicit`, `inferred`) | - | Hoch. Contract vorhanden; Runtime-Producer noch offen. |
