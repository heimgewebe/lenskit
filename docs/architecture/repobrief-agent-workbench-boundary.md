# RepoGround Agent Workbench Boundary

This document defines the RepoGround Agent Workbench boundary before the full Workbench exists.

It corrects one important naming distinction:

- **Agent Workbench** is the integrated, deterministic code-understanding tool layer that helps agents use RepoGround to improve code.
...
The Workbench may live inside RepoGround when it remains deterministic and read-only. The Sidecar must stay outside RepoGround because it mutates worktrees and runs commands.

## Purpose

RepoGround turns repository state into deterministic, citable context. The Agent Workbench builds on that context with structured code-intelligence tools so an agent can understand a repository, plan edits, and cite the evidence behind proposed changes.
...
- **RepoGround Snapshot**: A generated repository brief at a specific generation time. It may be stale relative to the live working tree, GitHub, or a pull request.
- **Canonical Brief Source**: The canonical Markdown content inside a RepoGround bundle. It is the content authority for the generated snapshot.
...
- **Agent Workbench**: The integrated RepoGround analysis/tool layer for agents. It may include AST-derived symbols, static references, relation cards, range/citation navigation, query helpers, reading-plan helpers, and other deterministic code-understanding tools.
...
- **Patch Evaluation Artifact**: A future external artifact emitted by the Patch Evaluation Sidecar. RepoGround may later read and link such artifacts, but must not produce or interpret them as approval.
- **Worktree**: A mutable checkout or workspace. Worktrees are outside the RepoGround read-only evidence layer.

## Boundary decision

RepoGround may grow an integrated Agent Workbench for deterministic code understanding.

RepoGround must not grow an internal mutation axis for patch, shell, test, Git, pull-request, deployment, or sandbox operations.

The accepted architecture is:

- RepoGround remains the evidence, snapshot, citation, and deterministic analysis layer.
- Agent Workbench tools may be integrated into RepoGround if they are explicit, deterministic, inspectable, and read-only.
...
1. **Understanding workbench**: inside RepoGround, because agents need better static evidence to edit well.
2. **Mutation/evaluation sidecar**: outside RepoGround, because applying patches and running commands changes the authority model.

## RepoGround responsibilities
...
When RepoGround reads external patch-evaluation artifacts, it may report their presence, provenance fields, availability, and declared status. It must not convert those observations into a release verdict.
...
RepoGround and its integrated Agent Workbench must not:
...
LLM-facing summaries may consume RepoGround evidence, but they are not the canonical evidence layer. The canonical layer remains deterministic and inspectable.
...
- consume RepoGround snapshots, Workbench outputs, and citations as read-only context,
...
- link observations back to RepoGround citations or source ranges,
- stop with a precise report when the workspace, command policy, secrets policy, or provenance is insufficient.

The Sidecar is useful precisely because it is outside the RepoGround core. It can be allowed to mutate an isolated workspace without weakening RepoGround's evidence boundary.

## Agent flow

A typical agent improvement flow is:

1. RepoGround creates or reads a snapshot of a repository state.
...
7. RepoGround may later read or link those Sidecar artifacts as external evidence.
...
- referenced RepoGround snapshot or bundle manifest,
...
RepoGround MCP remains a read-first boundary.

RepoGround MCP may later expose Agent Workbench resources and read-only tools when they are deterministic code-understanding surfaces. Examples include symbol lookup, range lookup, relation lookup, required-reading resolution, and query over existing indexes.

RepoGround MCP must not trigger Patch Evaluation Sidecar actions as a side effect of resource reads or read-only tools. It must not run shells, apply patches, create PRs, inspect secrets, or silently refresh snapshots.
...
If Sidecar control is ever exposed through MCP, it should be a separate Sidecar surface with its own authority model, not a hidden extension of RepoGround resources.

## Security and secret boundary

RepoGround should not require secrets to read existing bundles or expose snapshot evidence.
...
The Patch Evaluation Sidecar may need access to credentials, private dependencies, or privileged environments in some deployments. Those capabilities must be explicit, scoped, logged, redacted where necessary, and kept outside RepoGround's deterministic read path.
...
A successful RepoGround read, Workbench result, Sidecar run, CI job, review comment, or PR status does not by itself establish:
...
False. In RepoGround, the Agent Workbench is the integrated read-only code-understanding layer. Patch running belongs to the external Patch Evaluation Sidecar.

### “If RepoGround can locate relevant tests, the patch is good.”
...
False. RepoGround MCP is read-first. Shell access crosses into Sidecar authority and must not be smuggled through a read-only resource interface.
...
- possible confusion if Sidecar artifacts are displayed beside RepoGround artifacts without clear authority labels.
...
- forces RepoGround to own shell, secrets, rollback, Git, worktree, and runtime semantics,
...
The decision is not “tooling or no tooling.” The decision is “deterministic understanding tools inside RepoGround; mutable execution outside.” A good workbench has sharp tools, not a hidden flamethrower in the drawer.
