...
4. **Phase-5-Artefakte und Implementierungsstand:**
   Für `cross_repo_links` und `federation_conflicts` existieren eigene Contracts (`cross-repo-links.v1.schema.json`, `federation-conflicts.v1.schema.json`). `federation_conflicts` ist heuristisch/minimal im föderierten Query-/Trace-Pfad emittiert (`federation_query.py`) und per CLI-Trace persistierbar (`cmd_federation.py`); offen bleibt eine belastbare Identity- und Konfliktlogik. `cross_repo_links` ist als minimaler Co-Occurrence-Output im föderierten Query-Pfad emittiert und per CLI-Trace persistierbar. Semantische Grenze: keine Identitäts-/Abhängigkeitsaussage, sondern nur gemeinsame Query-Präsenz in finalen Ergebnissen. `federation_index.json` ist implementiert.
...
6. **Authority/Canonicality-Felder (Phase 1 + 3.5):**
   ...
   **Außerhalb des Bundle-Manifest-Pfads (keine Schema-Constraints):**
   ...
   Folgepunkte: Vollständige Authority/Canonicality-Annotation für `query_trace` und `query_context_bundle` (Phase 4); föderierte Artefakte (`cross_repo_links.json` heuristisch/minimal emittiert; `federation_conflicts.json` heuristisch/minimal emittiert, Phase 5) und klare Trennung zur zukünftigen Identity-Logik; `agent_query_session` ist als Phase-6-Runtime-Artefakt in diesem Inventar eingetragen (siehe Anmerkung 3).
