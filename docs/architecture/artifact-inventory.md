...
| `agent_query_session` (API Runtime) | Runtime Observation (Wrapper) | `runtime_observation` | `observation` | `service.app` via `retrieval.session.build_agent_query_session_v2` | Agents, Artifact Lookup (`/api/artifact_lookup`) | `merger/lenskit/contracts/agent-query-session.v2.schema.json` | Nein (Runtime Output) | Provenienz-Wrapper für Query-Kontext-Projektion; `artifact_shape`: `wrapper` |
...
5. **Authority/Canonicality-Felder (Phase 1 + 3.5):**
   ...
   **Außerhalb des Bundle-Manifest-Pfads (keine Schema-Constraints):**
   ...
   Folgepunkte: Vollständige Authority/Canonicality-Annotation für `query_trace` und `query_context_bundle` (Phase 4); föderierte Artefakte (`cross_repo_links.json`, `federation_conflicts.json`, Phase 5); `agent_query_session` ist als Phase-6-Runtime-Artefakt in diesem Inventar eingetragen (siehe Anmerkung 3).
