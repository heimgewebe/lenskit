...
| `<stem>.derived_manifest.json` | `derived_manifest_json` | `navigation_index` | `derived` | `core.merge` | Bundle-Konsumenten | - | Ja | Verlinkt abgeleitete Artefakte |
...
| `<stem>.architecture_graph.json`| `architecture_summary` | `diagnostic_signal` | `diagnostic` | `architecture.import_graph` | `architecture.graph_index` | `architecture.graph.v1.schema.json` | Ja | Erzeugung des Index |
...
| `<stem>.retrieval_eval.json` | `retrieval_eval_json` | _(Phase 1: nicht annotiert)_ | _(Phase 1: nicht annotiert)_ | `retrieval.eval_core` | CI, Entwickler | `retrieval-eval.v1.schema.json` | Nein | Evaluierungsmetriken |
...
| `<stem>.entrypoints.json` | `entrypoints` | _(Phase 1: nicht annotiert)_ | _(Phase 1: nicht annotiert)_ | `architecture.entrypoints` | `architecture.graph_index` | `entrypoints.v1.schema.json` | Ja | Berechnung des Graph-Boosts |
...
5. **Authority/Canonicality-Felder (Phase 1):**
   Die Felder `authority`, `canonicality`, `regenerable` und `staleness_sensitive` sind in `bundle-manifest.v1.schema.json` optional. Wo sie gesetzt sind, sind sie pro Rolle wertbeschränkt (z.B. darf `sqlite_index` keine `canonical_content`-Autorität tragen, `architecture_summary` keinen `content_source`-Status). Der Producer (`merger/lenskit/core/merge.py`, `AUTHORITY_REGISTRY`) emittiert diese Felder aktuell für sieben Rollen: `canonical_md`, `index_sidecar_json`, `dump_index_json`, `derived_manifest_json`, `chunk_index_jsonl`, `sqlite_index`, `architecture_summary`. Für `retrieval_eval_json`, `delta_json`, `graph_index_json` und Runtime-Artefakte folgen die Annotationen in späteren Phasen (3, 4, 5).
