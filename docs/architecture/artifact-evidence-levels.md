...
| forensic_strict | all diagnostic_full roles + claim_evidence_map_json | claim_evidence_map_available + strict capabilities | forensic preflight: canonical/chunk/citation/claim-map hash ok, claim-map schema valid, post-emit pass, no skipped required checks, explicit redaction policy | keine | any required skip; missing/invalid claim map | "Forensic strict (preflight-gated)" | "Forensic strict requires a PASS preflight; pass does not assert claim truth." |
