...
| Ruff-Scope: CI prüft nur `F401`/`F811`; ~120 Findings außerhalb des Gates, ehemals keine eingecheckte Ruff-Konfiguration (lokales `ruff check` ≠ CI-Verhalten) | `.github/workflows/lint.yml`; `ruff.toml`; `docs/proofs/ruff-scope-ratchet-v1-proof.md` | **behoben** | `ruff.toml` macht den bisherigen CI-Scope explizit; CI und lokale Anleitung nutzen `ruff check .`. Bewusst kein Style-Rundumschlag: breitere Ruff-Default-Funde bleiben außerhalb dieses Ratchets. |
