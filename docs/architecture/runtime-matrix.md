...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **`core.merge` (Ingestion)** | Repository Dateisystem, Profilkonfigurationen | Immer: `canonical_md`, `dump_index.json`, `index_sidecar.json`. Conditional: `.index.sqlite` und `retrieval_eval.json` (nur bei Retrieval/Dual-Output mit erzeugtem Chunk-Index); `citation_map_jsonl` (nur bei kohärentem Manifest und passendem `canonical_md` + `chunk_index_jsonl`); `output_health.json` (profil-/modusabhängig). | Ja (Bundle-Manifest + Sidecar) | Validiert Rollen gegen Constants. | CWD-Fallback (bei absolutem CWD-Mode). | Explizit. Fehler in der Extraktion (z.B. ParseError) werden geloggt, aber fortgesetzt. |
...
**Hinweis zu conditionals in `core.merge`:** Die Citation-Map-Emission erfolgt zweiphasig (provisorisches Manifest → Producer-Lauf → finales Manifest atomar neu schreiben); bei kohärentem Manifest bleibt ein Producer-Fehler fail-closed. `.index.sqlite` und `retrieval_eval.json` werden nur bei Retrieval/Dual-Output mit erzeugtem Chunk-Index erzeugt. `output_health.json` ist profil- und modusabhängig; ein präziser Codebeleg für den Garantiestatus fehlt noch und ist als offener Punkt zu behandeln.
