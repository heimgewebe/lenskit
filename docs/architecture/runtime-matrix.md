...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **`core.merge` (Ingestion)** | Repository Dateisystem, Profilkonfigurationen | `canonical_md`, `dump_index.json`, `index_sidecar.json`, `.index.sqlite`, `citation_map_jsonl` (bei kohärentem Manifest), `retrieval_eval.json`, `output_health.json` | Ja (Bundle-Manifest + Sidecar) | Validiert Rollen gegen Constants. | CWD-Fallback (bei absolutem CWD-Mode). | Explizit. Fehler in der Extraktion (z.B. ParseError) werden geloggt, aber fortgesetzt. |

> **Hinweis Citation-Map-Emission:** Die Erzeugung von `citation_map_jsonl` erfolgt zweiphasig: provisorisches Manifest → Producer-Lauf → finales Manifest wird atomar neu geschrieben. Bei kohärentem Manifest bleibt ein Producer-Fehler fail-closed.
