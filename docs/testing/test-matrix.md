...
| **rLens CLI Client** | `test_cli_rlens_client.py` (43 Tests) | Read-only HTTP-Client gegen rLens-Service-API. Invarianten: Base-URL-Priorität (`--base-url` > `RLENS_BASE_URL` > Default), Bearer-Header (`--token` > `RLENS_TOKEN`), Token-Redaction in Fehlerausgaben und SSE-Stream-Daten (Text und JSON), URL-/Query-Konstruktion für `health`/`artifacts`/`latest`/`jobs`/`job`/`logs`, `latest --repo` required, `jobs --limit` nur positive Werte, `--last-id` negative Werte werden an Server weitergegeben (Server clamps), SSE-Parser: `id:`/`event:`/`data:` Multiline, Kommentar-Skip, `event: end` terminiert Stream, HTTP/URL/Parse-Fehler Exit 1, Config-Fehler Exit 2, CLI-Fehler Exit 2, keine `requests`-Dependency. | **Belegt durch 43 Unit-Tests gegen monkeypatched `urlopen`; kein echter Server.** | Offen: `run`/`cancel` (PR E), Host-Profile (PR D), Auto-SSE-Reconnect, Remote-Betriebsentscheidung. |
