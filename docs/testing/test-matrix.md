...
| **Frontend Parity Gates (Content vs Diagnostic)** | `test_parity.py` (`test_tool_parity_contract_invariants`, `test_content_parity_gate_can_pass_without_retrieval_eval_json`, `test_content_parity_allows_equal_empty_fts`, `test_diagnostic_parity_gate_requires_diagnostic_artifacts_and_status`, `test_diagnostic_parity_fails_when_content_fails`, `test_diagnostic_parity_conditional_not_required_when_not_expected`, `test_diagnostic_parity_fails_when_fts_non_empty_expected_but_absent`, `test_diagnostic_parity_fails_when_retrieval_eval_expected_but_not_manifested`, `test_diagnostic_parity_fails_when_citation_map_expected_but_invalid`, `test_expected_flags_must_be_bool`, `test_parity_gates_require_strict_boolean_true`), `test_parity_state.py`, `test_cli_parity_compare.py`, `test_output_health.py` (`range_ref_resolution_status`-Faelle) | Trennt `content_parity_pass` (Datei-/Hash-/Chunk-/FTS-Inhalt) von `diagnostic_parity_pass` (Health-/Range-Ref-/Eval-/Manifest-Diagnosebedingungen). Content-Paritaet impliziert keine Diagnose-Paritaet. Edge Case: logisch gleich leere FTS-Inhalte brechen Content-Paritaet nicht. Bedingte Artefakte werden nur verlangt, wenn `*_expected=True`; nicht-bool `*_expected`-Werte scheitern fail-closed. `retrieval_eval_json` muss manifestiert (nicht nur vorhanden) sein. Neuer Real-Dump-Parser-/CLI-Basispfad fuehrt Bundle-Manifests in `evaluate_parity_gates` zusammen, behandelt unsichere Manifest-Artefaktpfade fail-closed und validiert `citation_map_jsonl` mit Contract-/Validator-Pruefung statt reiner JSONL-Formpruefung. Exit-Codes 0/1/2 sind getestet. | **Belegt durch Kerntests gegen `merger/lenskit/core/parity_gates.py` plus gehaertete Parser-/CLI-Basistests.** | Weiterhin offen: globale CLI-Erzwingung/CI-Gate als Blocker und vollstaendige E2E-Beweise aus echten repolens-vs-rlens Dump-Paaren je Profil. |
...
| **rLens CLI Client** | `test_cli_rlens_client.py` (44 Tests) | Read-only HTTP-Client gegen rLens-Service-API. Invarianten: Base-URL-Priorität (`--base-url` > `RLENS_BASE_URL` > Default), Bearer-Header (`--token` > `RLENS_TOKEN`), Token-Redaction in Fehlerausgaben und SSE-Stream-Daten (Text und JSON), URL-/Query-Konstruktion für `health`/`artifacts`/`latest`/`jobs`/`job`/`logs`, `latest --repo` required, `jobs --limit` nur positive Werte, `logs --timeout` nur positive Werte (lokale Config-Validierung vor `urlopen`), `--last-id` negative Werte werden an Server weitergegeben (Server clamps), SSE-Parser: `id:`/`event:`/`data:` Multiline, Kommentar-Skip, `event: end` terminiert Stream, HTTP/URL/Parse-Fehler Exit 1, Config-Fehler Exit 2, CLI-Fehler Exit 2, keine `requests`-Dependency. | **Belegt durch 44 Unit-Tests gegen monkeypatched `urlopen`; kein echter Server.** | Offen: `run`/`cancel` (PR E), Host-Profile (PR D), Auto-SSE-Reconnect, Remote-Betriebsentscheidung. |
