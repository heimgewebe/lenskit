...
| **rLens CLI Client** | `test_cli_rlens_client.py` (18 Tests) | Read-only HTTP-Client gegen rLens-Service-API. Invarianten: Base-URL-Priorität (`--base-url` > `RLENS_BASE_URL` > Default), Bearer-Header (`--token` > `RLENS_TOKEN`), Token-Redaction in Fehlerausgaben, URL-/Query-Konstruktion für `health`/`artifacts`/`latest`, `latest --repo` required, HTTP/URL/Parse-Fehler Exit 1, CLI-Fehler Exit 2, keine `requests`-Dependency. | **Belegt durch 18 Unit-Tests gegen monkeypatches `urlopen`; kein echter Server.** | Offen: `logs JOB_ID`/SSE (PR C), `jobs`/`job JOB_ID`, `run`/`cancel` (PR E), Host-Profile (PR D), Remote-Betriebsentscheidung. |
